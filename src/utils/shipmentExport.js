 ...
 * @param {string} templateType - 'fba', 'awd', or 'production-order'
 * @param {Array} products - Array of product objects with qty > 0
 * @param {Object} shipmentData - Shipment metadata (shipmentNumber, etc.)
 */
export const exportShipmentTemplate = async (templateType, products, shipmentData) => {
  // Filter products that have been added (qty > 0)
  const productsToExport = products.filter(p => p.qty > 0);
  
  if (productsToExport.length === 0) {
    alert('No products to export. Please add quantities to products first.');
    return;
  }

  try {
    if (templateType === 'awd') {
      await exportFromTemplate('/assets/AWD_Template.xlsx', productsToExport, shipmentData, 'AWD');
    } else if (templateType === 'fba') {
      await exportFromTemplate('/assets/FBA_Template.xlsx', productsToExport, shipmentData, 'FBA');
 ...
 */
const exportFromTemplate = async (templatePath, products, shipmentData, type) => {
  try {
    ...
    workbook.eachSheet((sheet) => {
      if (sheet.name.includes('template')) {
        worksheet = sheet;
      }
    });
 ...
 */
const createProductionOrderExport = async (products, shipmentData) => {
  const workbook = new ExcelJS.Workbook();
 ...
 */
const getDefaultUnitsPerBox = (size) => {
  if (!size) return 12;
