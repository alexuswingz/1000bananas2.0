    ...
    const fetchInventory = async () => {
      try {
        ...
        if (response.success) {
          ...
          const transformed = response.data.map(bottle => ({
            id: bottle.id,
            name: bottle.bottle_name || bottle.name,
            currentInv: bottle.warehouse_quantity || 0,
    ...
    <div className={`min-h-screen ${themeClasses.pageBg}`}>
      <div className="p-6">
        ...
        <div className="mb-4">
          <button
            type="button"
            onClick={() => navigate('/dashboard/supply-chain/bottles/cycle-counts')}
