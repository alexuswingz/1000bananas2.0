    ...
    const fetchInventory = async () => {
      try {
        ...
        if (response.success) {
          ...
          const transformed = response.data.map(closure => ({
            id: closure.id,
            name: closure.closure_name || closure.name,
            currentInv: closure.warehouse_quantity || 0,
