...
const CycleCountsTable = forwardRef(({ themeClasses, onCreateCount }, ref) => {
  ...
  const fetchCycleCounts = async () => {
    try {
      setLoading(true);
      const response = await boxesApi.getCycleCounts();
      if (response.success) {
  ...
  useImperativeHandle(ref, () => ({
    createCount: async (type) => {
      try {
        ...
        const response = await boxesApi.createCycleCount({
        ...
        if (response.success) {
          ...
          navigate('/dashboard/supply-chain/boxes/cycle-counts/detail', {
            state: { 
              countId: newCountId,
              countData: {
                id: newCountId,
            ...
            cycleCounts.map((count, index) => {
              ...
              >
                ...
                <td style={{ padding: '0.65rem 1rem', verticalAlign: 'middle' }}>
                  <button
                    type="button"
                    onClick={() => navigate('/dashboard/supply-chain/boxes/cycle-counts/detail', {
