...
const ActionItems = () => {
  ...
  const bg = isDarkMode ? 'bg-dark-bg-primary' : 'bg-light-bg-primary';
  const cardBg = isDarkMode ? 'bg-dark-bg-secondary' : 'bg-white';
  const text = isDarkMode ? 'text-dark-text-primary' : 'text-light-body';
  const textSecondary = isDarkMode ? 'text-dark-text-secondary' : 'text-gray-600';
  const border = isDarkMode ? 'border-dark-border-primary' : 'border-gray-200';

  return (
    <div className={`min-h-screen ${bg} ${text}`} style={{ background: isDarkMode ? '#0B111E' : undefined }}>
      <div className="p-6 space-y-6">
        {/* Toolbar: no header bar - same components inline on page background */}
        <div className="flex flex-wrap items-center justify-between gap-4 min-w-0">
          {/* Left: rocket icon + Action Items text + tabs (to the right of the text) */}
          <div className="flex items-center gap-4 min-w-0">
            ...
            >
              <img src="/assets/action-items-icon.png" alt="" className="w-5 h-5 object-contain" />
            </div>
            <span className="text-xl font-bold text-white truncate">Action Items</span>
            {/* Segment control - My Tickets / All */}
            <div
          ...
          {/* Right: search, New Action Item button, settings gear */}
        ...
        {/* KPI cards - dark cards with colored top border only */}
        ...
        {/* Table - dark theme, rows with spacing/gap between each data row */}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500 flex items-center gap-1.5 mt-0.5">
                            {row.productId}
                            <button type="button" className="p-0.5 rounded hover:bg-white/10 transition-colors" title="Copy">
      ...
      {/* Hide scrollbar but keep scroll behavior (modal body) */}
      <style>{`
      ...
      {/* New Action Item Modal - matches design */}
      {showNewActionModal && (
        <div
        ...
        >
          ...
          >
            ...
            {/* Modal body - fills space between header and footer, scrolls if needed (scrollbar hidden) */}
            <div className="action-items-modal-body-scroll flex-1 min-h-0 px-6 py-5 space-y-5 overflow-y-auto">
              {/* Select Product - sell products container: 552px fill, 41px height, 8px radius, 1px border, padding 12/16/12/16, gap 8 */}
              <div>
              ...
              {/* Category - containers: 126×63px, radius 8px, border 1px, padding 12px, gap 4px */}
              <div>
            ...
            {/* Modal footer - Cancel left, Create Shipment right */}
