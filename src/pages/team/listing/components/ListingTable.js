...
const ListingTable = ({ 
  ...
  const stages = [
    ...
    { key: 'aplus', label: 'A+' },
    { key: 'listingSetup', label: 'LISTING\nSETUP' },
    { key: 'pdpSetup', label: 'PDP\nSETUP' },
    { key: 'vine', label: 'VINE' },
  ];

  const handleProductClick = (product) => {
    // Navigate to product detail page with specific tabs to show
    navigate('/dashboard/products/catalog/detail', { 
      state: { 
        ...
        returnPath: '/dashboard/team/listing', // Return to Listing page when back button is clicked
        allowedTabs: ['essential', 'slides', 'aplus', 'finishedGoods', 'pdpSetup', 'vine'] // Only show these tabs
      } 
    });
  };
